...
function _getCSNAssociatedEntitySet(relationshipName, toRole, navPropName, mockServerUc) {
    ...
    if (!allAssociations[associationName]) {
        ...
        if (array != 'undefined' && array != null && array.length != null && array.length > 0) {
            differentAssociationNames = allAssociations[array[array.length - 1]];
        }
    } else {
        differentAssociationNames = allAssociations[associationName];
    }
    return _getCSNAssociatedRefrenentialConstraints(differentAssociationNames, toRole,
