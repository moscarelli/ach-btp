...
async function transpileDirectory(path, { compilerOptions, createFileOptions }, includeExclude = defaultIncludeExclude) {
    ...
    const allFiles2 = new glob_1.GlobSync(includes, {
        ignore: excludes,
        cwd: path
    }).found;
...
function parseModuleResolutionKind(input) {
    return input.includes('node')
        ? typescript_1.ModuleResolutionKind.NodeJs
        : typescript_1.ModuleResolutionKind.Classic;
}
function parseScriptTarget(input) {
    ...
    logger.warn(`The selected ES target ${input} is not found - Fallback es2019 used`);
